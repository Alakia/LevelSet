...
#define IMAGE			"/root/car.bmp"
#define MASK			"/root/carmask.bmp"


#define ITERATIONS   5000 //迭代次数
#define THRESHOLD	 170  //阈值
#define EPSILON		 35   //ε

#define ALPHA		 0.009 //α
#define DT			 0.25  //δ
...
void init_phi(){
	...
	mask = (unsigned char *)malloc(imageW*imageH*sizeof(unsigned char));//建立一个图象大小的字符型数组mask

	//printf("Init Mask\n");
	LoadBMPFile(&h_Mask, &imageW, &imageH, mask_path);//读图像


	for(r=0;r<imageH;r++){
		for(c=0;c<imageW;c++){
			mask[r*imageW+c] = (h_Mask[r*imageW+c].x)/255;//归一化图像灰度到 [0,1]
...
int main(int argc, char** argv){
	...
	LoadBMPFile(&h_Src, &imageW, &imageH, image_path);//读取图像
	D = (float *)malloc(imageW*imageH*sizeof(float));//开辟图象大小的浮点数矩阵D

	//printf("Input Image\n");
	for(r=0;r<imageH;r++){
		for(c=0;c<imageW;c++){
			D[r*imageW+c] = h_Src[r*imageW+c].x;//初始化矩阵D  将图像的灰度存入D
			/*printf("%3.0f ", D[r*imageW+c]);*/
		}
		//printf("\n");
	}

	N = imageW*imageH;//计算图像总点数N

	for(i=0;i<N;i++){
		D[i] = EPSILON - abs(D[i] - THRESHOLD);//对每一点计算  ε=35 - abs(D - THRESHOLD=170)
	}
